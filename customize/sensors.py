...
file_path = os.path.join(os.getcwd(), 'evaluation_outputs', 'city_exception_1.txt')
...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event) -> None:
        ...
        record = f"---------Exception_Collision---------\n" \
             f"Frame: {event.frame}\n" \
             f"Timestamp: {event.timestamp}\n" \
             f"Transform: Location({event.transform.location.x}, {event.transform.location.y}, {event.transform.location.z}) " \
             f"Rotation({event.transform.rotation.pitch}, {event.transform.rotation,yaw}, {event.transform.rotation.roll})\n" \
             f"Actor: {event.actor.type_id}\n" \
             f"Other Actor: {event.other_actor.type_id}\n" \
             f"Normal Impulse: ({event.normal_impulse.x}, {event.normal_impulse.y}, {event.normal_impulse.z})\n" \
             f"--------------------------\n"
            ...
            for wp in trigger_waypoints:
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and dot_ve_wp > 0:
                    ...
                    if self._is_vehicle_crossing_line(
                        ...
                        self._last_light = None
                        record = f"---------Exception_Lane_Crossing---------\n" \
                                 f"Lane Crossing Happened\n" \
                                 f"--------------------------\n"
                        with open(file_path, 'a') as f:
                            f.write(record)
